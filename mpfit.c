...
/* MODIFIED for use with maximum likelihood method */
...
/*		check = j*m+1;
		printf("attaching analytic derivs: %d %d %d %d %d\n", j, ifree[j], j, m, j*m);*/
...
/*	printf("deriv check: %e\n", fjac[check]);*/
  ...
  if (has_numerical_deriv) for (j=0; j<n; j++) {  /* Loop thru free parms */
	...
	/* BUG FIX: the ij+=m line did not exist before
	 * so the analytic derivs did not work at all
	 * now they do
	 * */
    if (dside && dsidei == 3) {
		ij += m; /* bug fix?*/
